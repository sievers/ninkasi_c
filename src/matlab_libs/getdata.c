...
typedef int SLIMFILE; 

SLIMFILE *slimopen(char *fnmae, char *mode)
{
  return NULL;
}

int slimseek(SLIMFILE *slim, int offset, int whence)
{
  return 0;
}

int slimread(void *buf, int n, int count, SLIMFILE *slim)
{
  return 0;

}

void slimclose(SLIMFILE *slim)
{
  return;
}

int slimrawsize(char *c)
{
  return 0;
}
...
static inline off_t seek_wrap(struct FileHandle *R, off_t offset, int whence) {
  if (R->fp >= 0)
    return lseek(R->fp, offset, whence);
  if (R->slim)
...
static int ReadLinterpFile(struct LinterpEntryType *E) {
  ...
  if (i<2) {
    MakeDummyLinterp(E);
    fclose(fp);  // added Feb 18, 2008 (Switzer)
    return (GD_E_OPEN_LINFILE);
  }
  E->n_interp = i;
